...
#sk("Frameworks",   "React, FastAPI, Spring Boot, FastMCP, LangGraph, LangChain, LangSmith, PyTorch")
...
- Built *generative code solutions* for developers — *API-aware templates*, *SDK patterns*, and *code-generation workflows* — to accelerate integration with platform *APIs* and improve correctness of *JavaScript/TypeScript* implementations
- Optimized prompt routing and agent concurrency to maximize *KV-cache hit rate*, reducing *end-to-end latency* by *60%* and lowering LLM API costs by *50%*, directly improving *rendering performance* and responsiveness
...
- Contributed to the development of an Identity & Access Management platform using *React*, *Spring Boot*, and *AWS*, building *extensible UI components* supporting thousands of users across 30+ applications with a focus on *backward compatibility*
- Designed and developed a cloud-agnostic Storage Service using *Python*, *PostgreSQL*, and *Redis* that seamlessly handles file uploads, downloads, and deletions across *Google Cloud Storage*, *S3*, and *MinIO* through expiring signed URLs, ensuring *resiliency* across multi-cloud environments
- Developed *backend workflows* using *AWS Step Functions* to handle long-running data import jobs and prevent *AWS Lambda* timeouts, improving *performance* and operational stability
- Built an internal support chatbot using *LangChain* and *RAG* (Retrieval-Augmented Generation) with *PostgreSQL* vector store, reducing average ticket resolution time by 20% via natural language queries over documentation
