...
- Architected a *concurrent multi-agent* orchestration system using *Python*, *LangChain* and *LangGraph*, applying *object-oriented* and *multithreaded* software design principles to enable reliable coordination across complex, parallel workflows
- Optimized *inter-thread communication* and agent concurrency to maximize *KV-cache hit rate*, reducing end-to-end latency by *60%* and lowering LLM API costs by *50%*
- Enabled a robust *observability layer* using *Langfuse* and *LangSmith* for *tracing*, *structured logs*, and *evaluation hooks* to *debug and troubleshoot* system failures and track quality over time
...
- Designed and developed a cloud-agnostic Storage Service using *Python*, *PostgreSQL*, and *Redis*, applying *object-oriented* design to seamlessly handle file uploads, downloads, and deletions in *Google Cloud Storage*, *S3*, and *MinIO* through expiring signed URLs
- Developed backend workflows using *AWS Step Functions* to handle long-running data import jobs and prevent *AWS Lambda* timeouts, ensuring reliable inter-service communication and correct execution sequencing
