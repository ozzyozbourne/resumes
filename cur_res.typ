...
- Designed a *hierarchical multi-agent system* for a multi-tenant *CRM* SaaS application using *LangGraph, LangChain, and FastAPI*, where a main orchestrator lazily loads *sub-agent catalogs* and delegates to *40+ specialized sub-agents*, each built per request with the user's *JWT, tenant, and role context* propagated end-to-end so every backend call respects the caller's permissions
- Compressed *~500 backend operations* into *tool-group dispatchers* allowing progressive disclosure of tools, collapsing 60+ REST endpoints per agent into a few typed entry points, *lifting correct tool selection ~90%* and cutting *sub-agent context bloat* and tool-call errors
...
- Built an *eval harness* that scored *LangGraph* agent runs with an evaluator and fed the top runs back into prompt tuning, lifting *tool-call success* by *~60%* over a single *LLM-judge* baseline

#work("Software Engineer", "Noida, India", "Qualitest", "Feb 2021 - Dec 2023")
- Built a *RAG* AI chatbot for a *K-12 education platform* using *LangChain* and *PostgreSQL pgvector* store, *improving customer engagement by 50%* by enabling natural-language queries over course content and platform documentation
...
- Turned a single user goal into a *dependency-aware task plan* that is automatically dispatched to a *fleet of AI coding agents running across multiple machines*, with built-in retries and timeout handling
